...
function FABigation() {
	...
	var BG = document.createElement("span"), L = document.createElement("div"), R = document.createElement("div");
		L.className = R.className = "FABdo"; 
		L.id = "LFAB"; setIO(0,L,0);
		R.id = "RFAB"; setIO(0,R,0);
	var B = document.createElement("div"), T = document.createElement("div");
		B.className = T.className = "FABdo";
		T.id = "TFAB"; setIO(0,T,0);
		B.id = "BFAB"; setIO(0,B,0);
	...
	function home() {window.scrollTo({top:0, behavior:"smooth"})} //(CSS.supports("-moz-hyphens","none")?"auto":"smooth")
	B.addEventListener("click", home, {passive: true});
	function reload() {location.reload();}
	T.addEventListener("click", reload, {passive: true});

	function atTop() {return +(window.pageYOffset < 3)}
...
// if (CSS.supports("-moz-hyphens","none")){
// 	function Bscroll(e) {if (atTop()) e.preventDefault();}
// 	B.addEventListener("wheel", Bscroll, {passive: false});}